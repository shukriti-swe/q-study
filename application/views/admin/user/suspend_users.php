	        ...
	        <div class="">
	          <div class="" role="tab" id="headingOne">
	            <h4 class="panel-title text-center"> 
	                ...
	                <button class="btn btn-success">Suspend Users</button>
	            </h4>
	          </div>
	      	</div>
	      </div>
	      <div class="section full_section">
	      	<div class="form-group row suspend_list">
	      		
	      		<?php 
	      		$i = 0;
	      		foreach ($suspend_user_info as $key => $value): ?>
	      			<?php if ($i % 10 == 0 ): ?>
	      				<?php if ($i > 0): ?>
	      					</div>
	      				<?php endif ?>
...
<script>
	$(document).ready(function(){
		$('#next-button').click(function(){
			var offset = $(this).val();
			$.ajax({
	            url:'Admin/next_suspend_users_list',
	            method: 'POST',
	            data:{'offset': offset},
	            success: function(data){
	                $('.suspend_list').html(data); 
