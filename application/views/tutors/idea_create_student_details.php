<?php $i=1; foreach($student_ans_details as $idea_details){
   // echo "<pre>";print_r($idea_details);die();
   // echo $idea_details['user_id'];die();
   ?>
                     ...
                     <div class="col-sm-6">
                        <div class="workout_menu" style=" padding-right: 15px;">
                           <ul>
                              ...
                              <li><a href="tutor_student_idea_setting/<?=$idea_details['question_id']?>/<?=$idea_details['idea_id']?>/<?=$idea_details['user_id']?>/<?=$idea_details['module_id']?>" style="cursor:pointer; background: red; color: #fff;" id="show_question"> Idea <?=$i?></a></li>
                           </ul>
                        </div>
...
//unsubscribe();  // stop listening
  ...
  function setAnswer(){
    ...
    $.ajax({
    type: 'POST',
    url: 'tutor/teacher_check_workout',
    data: {
      imageData: imageData,student_id:student_id,idea_id:idea_id,idea_no:idea_no,tutor_id:tutor_id,question_id:question_id,module_id:module_id,
    },
    dataType: 'html',
      success: function (results) {

         location.href = 'tutor/idea_create_student_report/'+results; 
